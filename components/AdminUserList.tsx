...
export function AdminUserList({ users }: { users: User[] }) {
  ...
  const [pendingAssignments, setPendingAssignments] = useState<
    Record<string, { role: UserRole; tipo: SolicitanteType }>
  >({});
  ...
  const handleApprove = async (id: string) => {
    const assignment = pendingAssignments[id] || {
      role: "SOLICITANTE",
      tipo: "Docente",
    };
      ...
      {pendingUsers.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 shadow-sm">
          ...
          <p className="text-xs text-amber-750 mb-4">
            Novos solicitantes cadastrados aguardando atribuição de perfil e
            aprovação regulamentar para acesso ao sistema.
          </p>
          <div className="bg-white rounded-lg border border-amber-150 overflow-hidden shadow-xs overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {pendingUsers.map((u) => {
                  const assignment = pendingAssignments[u.id] || {
                    role: "SOLICITANTE",
                    tipo: u.tipo || "Docente",
                  };
                  return (
                    <tr
                      key={u.id}
                      className="hover:bg-amber-50/20 origin-left transition-all"
                    >
                            ...
                            <Select
                              ...
                              onChange={(e) =>
                                handleUpdateAssignment(
                                  u.id,
                                  assignment.role,
                                  e.target.value as SolicitanteType,
                                )
                              }
                              className="h-8 text-xs bg-white border border-rose-300 rounded font-medium select-none cursor-pointer"
                            >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200">
          ...
          <p className="text-xs text-slate-550 mt-0.5">
            Gestão de solicitantes, administradores e motoristas aprovados no
            sistema.
          </p>
        </div>
                      ...
                      (editData.role === "SOLICITANTE" ||
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setEditData({
                              ...editData,
                              tipo: e.target.value as SolicitanteType,
                            })
                          }
                          className="h-7 text-sm px-2"
                        >
                        ...
                        <div className="flex justify-end gap-2 text-xs">
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              if (
                                window.confirm(
                                  'Deseja resetar a senha deste usuário para "1234"?',
                                )
                              ) {
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              if (
                                window.confirm(
                                  "Tem certeza que deseja excluir este usuário definitivamente?",
                                )
                              ) {
